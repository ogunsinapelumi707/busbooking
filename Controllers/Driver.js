...
export const Register = async (req, res, next) => {
  try {
    upload(req, res, async (err) => {
      ...
      const { fullname, phone, email, password, busId } = req.body;
      ...
      if (driverExist) {
        return res.json({ success: false, message: "Driver already exists with the given email" });
      }
...
export const Login = async (req, res, next) => {
  try {
    ...
    const token = Jwt.sign(
      { id: user._id, username: user.email, profileImage:user.profileImage, isAdmin: user.isAdmin },
